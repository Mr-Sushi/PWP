...
from ..models import Event, User, Following
...
class EventsByUser(Resource):
    def get(self, user_id):
        ...
        try:
            # find all the events
            followed_events = Following.query.filter_by(user=user_id).event.all()
...
class UsersByEvent(Resource):
    def get(self, event_id):
        ...
        try:
            # find all the users
            followers = following.query.filter_by(event = event_id).user.all()
